...
const BookCarousel = ({ title, bookMap = [] }) => {
  ...
  useEffect(() => {
    ...
    const interval = bookMap.length > 0 ? setInterval(() => {
      setCurrentPage((prev) => {
  ...
  if (!bookMap || bookMap.length === 0) {
    return (
      <div className="book-carousel-container" ref={containerRef}>
        ...
        <p style={{ textAlign: 'center' }}>No books available in this category</p>
      </div>
    );
  }
        ...
        <div className="book-carousel-content">
          ...
          >
            {bookMap.map((book, index) => {
              ...
              const id = volumeInfo.industryIdentifiers?.[0]?.identifier || "N/A";
              const title = volumeInfo.title || "Untitled";
              const authors = volumeInfo.authors?.join(', ') || "Unknown";
              const price = book.saleInfo?.retailPrice?.amount ? (book.saleInfo.retailPrice.amount * 290) : "N/A";
              const image = volumeInfo.imageLinks?.thumbnail || "/bookcover.jpg";
              ...
              return (
                ...
                >
                  ...
                  <div className="book-carousel-buttons">
                    <button className="book-carousel-addcart-button">Add to Cart</button>
                    <button className="book-carousel-buynow-button">Buy Now</button>
                  </div>
                </div>
              );
            })}
